    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            if self.use_augmentations:
                # For BYOL, we need two differently augmented views of the same image
                t1 = self.transform(image)
                t2 = self.transform(image)
                return t1, t2, label
            else:
...
class BYOL(nn.Module):
    def __init__(self, backbone=None, projection_dim=2048, prediction_dim=512, output_dim=128):
        super().__init__()
        ...
        feature_dim = 2048  # For ResNet50
        
        # Projector and predictor networks
        self.projector = MLP(feature_dim, projection_dim, output_dim)
        self.predictor = MLP(output_dim, prediction_dim, output_dim)
        
        # Create momentum encoder (teacher)
        self.momentum_backbone = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1)
        self.momentum_backbone.fc = nn.Identity()
        self.momentum_projector = MLP(feature_dim, projection_dim, output_dim)
    ...
    def _initialize_momentum_network(self):
        """Initialize momentum network as a copy of the online network"""
        for param_online, param_momentum in zip(self.backbone.parameters(), self.momentum_backbone.parameters()):
            param_momentum.data.copy_(param_online.data)
            param_momentum.requires_grad = False
            
        for param_online, param_momentum in zip(self.projector.parameters(), self.momentum_projector.parameters()):
            param_momentum.data.copy_(param_online.data)
            param_momentum.requires_grad = False
    
    @torch.no_grad()
    def update_momentum_network(self):
        """Update momentum network parameters with EMA"""
        for param_online, param_momentum in zip(self.backbone.parameters(), self.momentum_backbone.parameters()):
            param_momentum.data = param_momentum.data * self.m + param_online.data * (1. - self.m)
    ...
    def forward(self, x, get_features=False):
        ...
        return predictions, projections, features
    
    @torch.no_grad()
    def momentum_forward(self, x):
        features = self.momentum_backbone(x)
        projections = self.momentum_projector(features)
        return projections
...
def train(fold, class_names, num_classes, debug=False, batch_size=32, num_epochs=30):
    """
    Train the BYOL model on a single fold
    ...
    results_dir = f"results_byol_{timestamp}"
    ...
    optimizer = torch.optim.Adam(
        list(byol.parameters()) + list(classifier.parameters()),
        lr=3e-4
    )
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (x1, x2, labels) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}")):
            ...
            online_pred1, online_proj1, features1 = byol(x1)
            online_pred2, online_proj2, features2 = byol(x2)
            
            # Get momentum projections
            with torch.no_grad():
                target_proj1 = byol.momentum_forward(x1)
                target_proj2 = byol.momentum_forward(x2)
            
            # BYOL loss (symmetric)
            loss_1 = byol_loss_fn(online_pred1, target_proj2.detach())
            loss_2 = byol_loss_fn(online_pred2, target_proj1.detach())
            byol_loss = (loss_1 + loss_2) / 2
            
            # Classification loss
            with torch.no_grad():
                # Use the backbone to get features with no gradient history
                features1_clean = byol.backbone(x1)
                features2_clean = byol.backbone(x2)
            
            # Make predictions using these features
            logits1 = classifier(features1_clean.detach())
            logits2 = classifier(features2_clean.detach())
            
            cls_loss1 = classification_criterion(logits1, labels)
            cls_loss2 = classification_criterion(logits2, labels)
            cls_loss = (cls_loss1 + cls_loss2) / 2
            
            # Total loss
            loss = byol_loss + cls_loss
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Train BYOL on spectrograms')
...
class SpectrogramClassifier:
    def __init__(self, checkpoint_path, class_names, device=None):
        ...
        self.byol = BYOL(backbone).to(self.device)
        ...
        self.byol.load_state_dict(checkpoint['byol'])
    ...
    def predict(self, img_path, verbose=True):
        ...
        # Get prediction
        with torch.no_grad():
            features = self.byol.backbone(img_tensor)
...
def evaluate_fold(model_path, fold, class_names):
    ...
    for i in tqdm(range(len(val_ds))):
        ...
        # Get prediction
        with torch.no_grad():
            features = classifier.byol.backbone(img_tensor)
